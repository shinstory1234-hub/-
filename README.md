...
ADMIN_EMAIL=ex)admin@gmail.com
...
2. Run `supabase functions deploy track`.
3. Verify function URL `/functions/v1/track` is reachable.

## Track Function Test Flow
1. Local: `npm run dev`.
3. Local Post detail refresh 3 times, confirm `/api/track-post` `POST 200` JSON each time.
4. Deploy with Vercel env set and redeploy.
6. Deploy Post detail refresh 5 times, confirm view_count increases without reset.
